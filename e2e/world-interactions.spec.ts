...
test('uses classroom seats and drives a parked car end to end', async ({
  page,
}, testInfo) => {
  ...
  await page.evaluate(() =>
    window.__blockBuddiesE2E!.prepareClassroomSeatInteraction(),
  )
  await page.waitForTimeout(900)
  await expect(page.getByText('Ms Maple')).toBeVisible({ timeout: 15_000 })
  await expect(page.getByTestId('classroom-whiteboard')).toContainText(
    'Build a kind community',
  )
  const roomZoom = page.getByLabel('Room camera zoom')
  await expect(roomZoom).toBeVisible()
  await roomZoom.fill('1.75')
  await expect(roomZoom).toHaveValue('1.75')
  ...
  const chairAction = page
    .getByRole('button', { name: 'Sit on Classroom chair' })
    .first()
  ...
  expect(afterDrive[0]).toBeGreaterThan(beforeDrive[0])
  ...
  await page.screenshot({
    path: testInfo.outputPath('parking-drive-desktop.png'),
  })
...
test.describe('mobile vehicle controls', () => {
  ...
  test('replaces run and jump actions with car and brake controls while driving', async ({
    page,
  }, testInfo) => {
    test.setTimeout(90_000)
    await completeStartFlow(page, 'MobileDriver')
    await page.evaluate(() =>
      window.__blockBuddiesE2E!.prepareParkingInteraction(),
    )
    ...
    await expect(
      page.getByRole('button', { name: 'Toggle emotes' }),
    ).toBeHidden()
    ...
    await page.screenshot({
      path: testInfo.outputPath('parking-drive-mobile.png'),
    })
  })
})
...
test('starts the obby on top of its solid platform and allows movement', async ({
  ...
  expect(
    Math.hypot(afterMove[0] - beforeMove[0], afterMove[2] - beforeMove[2]),
  ).toBeGreaterThan(0.15)
})

test('completes the beginner obby and rewards the player', async ({ page }) => {
  test.setTimeout(90_000)
  await completeStartFlow(page, 'ObbyWinner')
  await expect(page.getByText('Loading town...')).toBeHidden({
    timeout: 30_000,
  })
  ...
  expect(result.obbyActive).toBe(false)
  expect(result.obbyFinished).toBe(true)
  expect(result.coins).toBeGreaterThanOrEqual(110)
  expect(result.earnedBadges).toContain('obby-rookie')
  expect(result.chatTexts.at(-2)).toContain('Obby complete!')
  expect(
    result.questProgress.find((quest) => quest.id === 'beginner-obby'),
  ).toMatchObject({
    completed: true,
    progress: 1,
  })
