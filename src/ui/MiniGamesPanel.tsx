import { Clock3, Coins, Gamepad2, MapPin, Play, RotateCcw, Trophy } from 'lucide-react'
...
export function MiniGamesPanel() {
  ...
  const activeDefinition = miniGame.activeId ? miniGameDefinition(miniGame.activeId) : undefined
    ...
    <Panel title="Mini Games">
      <div className="mb-3 rounded-2xl bg-gradient-to-br from-sky-100 to-blue-50 p-3 shadow-inner">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="font-black text-slate-950">Original blocky activities</h3>
            <p className="text-sm font-bold text-slate-600">
              Pick a fast offline activity, earn coins, and let the buddies cheer you on.
            </p>
          </div>
        </div>
        {activeDefinition ? (
          <div className="mt-3 rounded-xl bg-white/90 p-3 text-sm font-black text-slate-800 shadow">
            ...
            {miniGame.status === 'running' ? (
              <button type="button" onClick={cancelMiniGame} className="mt-3 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 text-white">
      ...
      <div className="space-y-3">
        {miniGameDefinitions.map((game) => {
          ...
          const isActive = miniGame.activeId === game.id && miniGame.status === 'running'
          return (
            <article key={game.id} className="overflow-hidden rounded-2xl bg-white shadow">
              <div className={`h-2 bg-gradient-to-r ${gameTones[game.id]}`} />
              <div className="p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="text-lg font-black text-slate-950">{game.title}</h3>
                    <p className="text-sm font-bold text-slate-600">{game.description}</p>
                  </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-black text-slate-600">
                  ...
                  <span className="rounded-lg bg-blue-50 px-2 py-1 text-blue-800">Plays {record?.plays ?? 0}</span>
        ...
        <article className="rounded-2xl bg-slate-950 p-3 text-white shadow-xl">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm font-bold text-slate-300">Classic platform course with checkpoints and a finish reward.</p>
            </div>
            <Trophy size={24} className="shrink-0 text-amber-300" aria-hidden />
          </div>
          <button
            type="button"
