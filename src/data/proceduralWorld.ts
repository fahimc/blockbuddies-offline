...
export function generateProceduralWorld({
  ...
  viewDistance,
  night,
}: ProceduralWorldInput): ProceduralWorld {
...
function generateChunk(
  ...
  const {
    centerX,
    centerZ,
    ownedHorizontalRoadCenters,
    ownedVerticalRoadCenters,
  } = plan.layout
  const pieces: ProceduralPiece[] = [
    piece(
      `ground:${cx}:${cz}`,
      'ground',
      [centerX, -0.12, centerZ],
      [chunkSize, 0.12, chunkSize],
      '#6fde6a',
    ),
  ]
  let buildingCount = 0
  ...
  for (const roadZ of ownedHorizontalRoadCenters) {
    pieces.push(
      piece(
        ...
        'road',
        [centerX, 0.01, roadZ],
        [chunkSize, 0.08, realScale.roadTile],
        '#9ca3af',
      ),
    )
    pieces.push(
      piece(
        ...
        'line',
        [centerX, 0.065, roadZ],
        [chunkSize, 0.025, 0.18],
        '#fde047',
      ),
    )
    pieces.push(
      piece(
        `pavement-x-a:${cx}:${cz}:${roadZ}`,
        'pavement',
        [centerX, 0.035, roadZ - sidewalkOffset],
        [chunkSize, 0.055, realScale.pavementWidth],
        '#e5e7eb',
      ),
    )
    pieces.push(
      piece(
        `pavement-x-b:${cx}:${cz}:${roadZ}`,
        'pavement',
        [centerX, 0.035, roadZ + sidewalkOffset],
        [chunkSize, 0.055, realScale.pavementWidth],
        '#e5e7eb',
  ...
  for (const roadX of ownedVerticalRoadCenters) {
    pieces.push(
      piece(
        ...
        'road',
        [roadX, 0.015, centerZ],
        [realScale.roadTile, 0.09, chunkSize],
        '#94a3b8',
      ),
    )
    pieces.push(
      piece(
        ...
        'line',
        [roadX, 0.07, centerZ],
        [0.18, 0.025, chunkSize],
        '#fde047',
      ),
    )
    pieces.push(
      piece(
        `pavement-z-a:${cx}:${cz}:${roadX}`,
        'pavement',
        [roadX - sidewalkOffset, 0.04, centerZ],
        [realScale.pavementWidth, 0.055, chunkSize],
        '#e5e7eb',
      ),
    )
    pieces.push(
      piece(
        `pavement-z-b:${cx}:${cz}:${roadX}`,
        'pavement',
        [roadX + sidewalkOffset, 0.04, centerZ],
        [realScale.pavementWidth, 0.055, chunkSize],
        '#e5e7eb',
  ...
  plan.parcels.forEach((parcel, index) => {
    ...
    const floors =
      parcel.use === 'commercial' ? 2 : 2 + Math.floor(random() * 2)
  ...
  if (random() > 0.76 && sidewalkPoints.length > 0) {
    const phonePosition =
      sidewalkPoints[Math.floor(random() * sidewalkPoints.length)]
    pieces.push(
      piece(
        `phone:${cx}:${cz}`,
        'phone-box',
        [phonePosition[0], realScale.phoneBoxHeight / 2, phonePosition[2]],
        [
          realScale.phoneBoxWidth,
          realScale.phoneBoxHeight,
          realScale.phoneBoxWidth,
        ],
        '#dc2626',
      ),
  ...
  if ((night || random() > 0.58) && sidewalkPoints.length > 0) {
    ...
    if (sidewalkPoints[2])
      addLamp(pieces, `lamp:${cx}:${cz}:b`, sidewalkPoints[2], night)
  }

  return {
    pieces,
    buildingCount,
    buildableParcels: plan.parcels.filter(
...
export function applyProceduralPlacementRules(pieces: ProceduralPiece[]) {
  ...
  const surfaces = pieces.filter((candidate) =>
    surfaceKinds.has(candidate.kind),
  )
  ...
  const placeGroup = (
    ...
    members.forEach((member) => {
      placed.push({
        ...member,
        position: [
          member.position[0] + dx,
          member.position[1],
          member.position[2] + dz,
        ],
      })
    })
  }
  ...
  pieces
    .filter(
      (candidate) =>
        ...
        !candidate.id.startsWith('landmark:town-hall:'),
    )
    .forEach((landmark) => {
      occupancy.reserve({
        id: landmark.id,
  ...
  pieces
    ...
    .sort(
      (a, b) =>
        Number(b.id.startsWith('landmark:')) -
        Number(a.id.startsWith('landmark:')),
    )
    .forEach((building) => {
      const members = pieces.filter(
        (candidate) =>
          candidate.id === building.id ||
          candidate.id.startsWith(`${building.id}:`),
      )
      placeGroup(
        building,
        'building',
        members,
        orientedFootprintScale(building),
      )
    })
  ...
  pieces
    ...
    .forEach((trunk) => {
      ...
      const members = pieces.filter(
        (candidate) =>
          candidate.id === `${baseId}:trunk` ||
          candidate.id === `${baseId}:top`,
      )
      placeGroup(trunk, 'tree', members, [
  ...
  pieces
    ...
    .forEach((post) => {
      ...
      const members = pieces.filter(
        (candidate) =>
          candidate.id === `${baseId}:post` ||
          candidate.id === `${baseId}:light`,
      )
      placeGroup(post, 'lamp', members, [
        Math.max(0.8, post.scale[0]),
        post.scale[1],
        Math.max(0.8, post.scale[2]),
      ])
    })
...
function removeSurfaceBlockerOverlaps(pieces: ProceduralPiece[]) {
  ...
  pieces.forEach((piece) => {
    ...
    const blocksRoad = driveCorridors.some((surface) =>
      overlapsTopDown(piece, surface, 0.04),
    )
    ...
    if (piece.kind === 'tree-trunk' || piece.kind === 'tree-top')
      blockedTreePrefixes.add(piece.id.replace(/:(trunk|top)$/, ''))
    else if (piece.kind === 'lamp-post' || piece.kind === 'lamp-light')
      blockedLampPrefixes.add(piece.id.replace(/:(post|light)$/, ''))
  ...
  return pieces.filter((piece) => {
    ...
    if (
      (piece.kind === 'tree-trunk' || piece.kind === 'tree-top') &&
      blockedTreePrefixes.has(piece.id.replace(/:(trunk|top)$/, ''))
    )
      return false
    if (
      (piece.kind === 'lamp-post' || piece.kind === 'lamp-light') &&
      blockedLampPrefixes.has(piece.id.replace(/:(post|light)$/, ''))
    )
      return false
    return true
  })
...
function removeDoorBlockerOverlaps(pieces: ProceduralPiece[]) {
  ...
  pieces.forEach((piece) => {
    if (
      !isDoorBlocker(piece) ||
      !doorZones.some((zone) => overlapsTopDown(piece, zone, 0.04))
    )
      return
    if (piece.kind === 'tree-trunk')
      blockedTreePrefixes.add(piece.id.replace(/:trunk$/, ''))
    else if (piece.kind === 'lamp-post')
      blockedLampPrefixes.add(piece.id.replace(/:post$/, ''))
    else blockedPieceIds.add(piece.id)
  })
...
function addBuilding(
  ...
  pieces.push(
    piece(
      ...
      [
        position[0],
        position[1] + scale[1] / 2 + realScale.roofHeight / 2,
        position[2],
      ],
      [scale[0] * 1.08, realScale.roofHeight, scale[2] * 1.08],
      roofColor,
      rotation,
    ),
  )
  pieces.push(
    piece(
      ...
      'door',
      [
        position[0] + frontOffset[0],
        realScale.doorHeight / 2,
        position[2] + frontOffset[1],
      ],
      [realScale.doorWidth, realScale.doorHeight, realScale.doorDepth],
      '#7c2d12',
      rotation,
    ),
  )
  ...
  for (let row = 0; row < Math.min(windowRows, 6); row += 1) {
    ...
    pieces.push(
      piece(
        `${id}:win:${row}:l`,
        'window',
        [position[0] + left[0], y, position[2] + left[1]],
        [realScale.windowWidth, realScale.windowHeight, realScale.windowDepth],
        '#dbeafe',
        rotation,
        '#93c5fd',
        0.12,
    ...
    pieces.push(
      piece(
        `${id}:win:${row}:r`,
        'window',
        [position[0] + right[0], y, position[2] + right[1]],
        [realScale.windowWidth, realScale.windowHeight, realScale.windowDepth],
        '#dbeafe',
        rotation,
        '#93c5fd',
...
function addTree(pieces: ProceduralPiece[], id: string, [x, , z]: Vec3) {
  pieces.push(
    piece(
      `${id}:trunk`,
      'tree-trunk',
      [x, realScale.treeTrunkHeight / 2, z],
      [0.32, realScale.treeTrunkHeight, 0.32],
      '#92400e',
    ),
  )
  pieces.push(
    piece(
      `${id}:top`,
      'tree-top',
      [x, realScale.treeTrunkHeight + realScale.treeCanopySize * 0.42, z],
      [
        realScale.treeCanopySize,
        realScale.treeCanopySize,
        realScale.treeCanopySize,
...
function addLamp(
  ...
  pieces.push(
    piece(
      `${id}:post`,
      'lamp-post',
      [x, realScale.lampHeight / 2, z],
      [0.14, realScale.lampHeight, 0.14],
      '#0f172a',
    ),
  )
  pieces.push(
...
function buildLandmarks(night: boolean): ProceduralPiece[] {
  ...
  addBuilding(
    pieces,
    'landmark:town-hall',
    [0, buildingHeightForFloors(3) / 2, -34],
    [meters(5.8), buildingHeightForFloors(3), meters(5.2)],
    '#d6a06a',
    '#2563eb',
  )
  pieces.push(
    piece(
      'landmark:town-hall:clock-tower',
      'landmark',
      [0, buildingHeightForFloors(3) + buildingHeightForFloors(2) / 2, -34],
      [meters(1.6), buildingHeightForFloors(2), meters(1.6)],
      '#c08457',
    ),
  )
  pieces.push(
    piece(
      'landmark:town-hall:clock-face',
      'window',
      [0, buildingHeightForFloors(4.25), -32.96],
      [1.08, 1.08, 0.08],
      '#f8fafc',
      undefined,
      '#fde68a',
...
function chunkRange(center: Vec3, viewDistance: 1 | 2 | 3): [number, number][] {
  ...
  for (let x = cx - viewDistance; x <= cx + viewDistance; x += 1) {
    for (let z = cz - viewDistance; z <= cz + viewDistance; z += 1)
      chunks.push([x, z])
  }
  return chunks
}
